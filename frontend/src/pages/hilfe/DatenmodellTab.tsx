...
function NetzplanDiagram() {
  ...
  const defs: T[] = [
    ...
    { id:'dokument_szenen', g:'core', f:[['id','UUID PK','Szenen-Instanz'],['werkstufe_id','UUID FK','Werkstufe','werkstufen'],['scene_identity_id','UUID FK','Stabile SZ-ID','scene_identities'],['sort_order','INT','Reihenfolge'],['scene_nummer','INT','Szenennummer'],['scene_nummer_suffix','VARCHAR','a, b (WGA)'],['ort_name','TEXT','Motiv-Anzeigetext'],['motiv_id','UUID FK','Verknüpftes Motiv','motive'],['int_ext','TEXT','INT|EXT|INT/EXT'],['tageszeit','TEXT','TAG|NACHT|ABEND'],['spieltag','INT','Drehtag-Index'],['spielzeit','TEXT','Spielzeit-Info'],['zusammenfassung','TEXT','Kurzbeschreibung'],['szeneninfo','TEXT','Redaktionelle Hinweise'],['seiten','TEXT','Seitenzahl (2 5/8)'],['dauer_min','INT','Dauer Min (Legacy)'],['dauer_sek','INT','Dauer Sek (Legacy)'],['is_wechselschnitt','BOOL','Legacy WS-Flag'],['sondertyp','TEXT','wechselschnitt|stockshot|flashback'],['stockshot_kategorie','TEXT','ortswechsel|zeit_vergeht|stimmungswechsel'],['stockshot_neu_drehen','BOOL','Neu zu drehen?'],['flashback_referenz_id','UUID FK','Ursprungsszene','scene_identities'],['content','JSONB','ProseMirror JSON'],['format','TEXT','Editor-Typ'],['stoppzeit_sek','INT','Spieldauer Sek'],['geloescht','BOOL','Soft-Delete'],['yjs_state','BYTEA','Yjs Collab State'],['updated_by','TEXT','Letzter Bearbeiter'],['updated_at','TSTZ','Letzte Änderung']]},
      ...
      <GroupBody id="core">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <TableCard title="dokument_szenen" color={C.green} note="Kreuzungstabelle: Content pro Szene pro Werkstufe (N:M)" fields={[
            ...
            { name: 'ort_name', type: 'TEXT', desc: 'Motiv-Anzeigetext "Drehort / Motiv / Untermotiv" (abgeleitet aus motiv_id)' },
            { name: 'motiv_id', type: 'UUID FK', desc: '-> motive.id (verknüpftes Motiv, seit v54)' },
            ...
            { name: 'sondertyp', type: 'TEXT', desc: "NULL | 'wechselschnitt' | 'stockshot' | 'flashback' (Stockshot ⇒ Label \"(Stockshot/Archivbild)\" hinter Motiv abgeleitet)" },
