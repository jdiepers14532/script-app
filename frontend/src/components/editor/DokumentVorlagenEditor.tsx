...
interface DokumentVorlagenEditorProps {
  ...
  readOnly?: boolean
  /** URL of the production logo to insert (from produktion.serienwerft.studio) */
...
function ZoneToolbar({ editor }: { editor: Editor | null }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 3, padding: '4px 8px', borderBottom: '1px solid var(--border)', background: 'var(--bg-subtle)', flexWrap: 'wrap', alignItems: 'center' }}>
      ...
      {btn('≡C', editor?.isActive({ textAlign: 'center' }) ?? false, () => editor?.chain().focus().setTextAlign('center').run(), 'Zentriert')}
...
// ── Image toolbar ─────────────────────────────────────────────────────────────
function ImageToolbar({ editor, produktionsLogoUrl }: {
  editor: Editor | null
  produktionsLogoUrl?: string | null
}) {
  ...
  const loadProdLogo = () => {
    ...
    insertImageSrc(produktionsLogoUrl)
  }
...
export default function DokumentVorlagenEditor({
  ...
  const [kzEditor, setKzEditor] = useState<Editor | null>(null)
  const [bodyEditor, setBodyEditor] = useState<Editor | null>(null)
  const [fzEditor, setFzEditor] = useState<Editor | null>(null)
    ...
    <div style={{ marginBottom: 8 }}>
      ...
      {value.kopfzeile_aktiv && (
        <div style={{ border: '1px solid #007AFF44', borderRadius: 6, overflow: 'hidden' }}>
          <ZoneToolbar editor={kzEditor} />
          <ImageToolbar editor={kzEditor} produktionsLogoUrl={produktionsLogoUrl} />
          <div style={{ display: 'flex', gap: 6, padding: '6px 8px', background: 'var(--bg-subtle)', borderBottom: '1px solid var(--border)' }}>
            <PlaceholderPalette editor={kzEditor} zone="kopfzeile" label="Platzhalter:" />
          </div>
          <div style={{ padding: '8px 12px', minHeight: 48, background: '#007AFF08' }}>
            <ZoneEditor
  ...
  const BodyZone = !noBody ? (
    <div style={{ marginBottom: 8 }}>
      ...
      <div style={{ border: '1px solid #00C85344', borderRadius: 6, overflow: 'hidden' }}>
        <ZoneToolbar editor={bodyEditor} />
        <ImageToolbar editor={bodyEditor} produktionsLogoUrl={produktionsLogoUrl} />
        <div style={{ display: 'flex', gap: 6, padding: '6px 8px', background: 'var(--bg-subtle)', borderBottom: '1px solid var(--border)' }}>
          <PlaceholderPalette editor={bodyEditor} zone="alle" label="Platzhalter:" />
        </div>
    ...
    <div>
      ...
      {value.fusszeile_aktiv && (
        <div style={{ border: '1px solid #FF950044', borderRadius: 6, overflow: 'hidden' }}>
          <ZoneToolbar editor={fzEditor} />
          <ImageToolbar editor={fzEditor} produktionsLogoUrl={produktionsLogoUrl} />
          <div style={{ display: 'flex', gap: 6, padding: '6px 8px', background: 'var(--bg-subtle)', borderBottom: '1px solid var(--border)' }}>
            <PlaceholderPalette editor={fzEditor} zone="fusszeile" label="Platzhalter:" />
          </div>
          <div style={{ padding: '8px 12px', minHeight: 48, background: '#FF950008' }}>
            <ZoneEditor
