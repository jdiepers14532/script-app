...
router.post('/job-kategorien', async (req, res) => {
  ...
  const result = await pool.query(
    ...
    [
      ...
      Array.isArray(praesenz_wochen) ? praesenz_wochen : [1],
...
router.put('/job-kategorien/:id', async (req, res) => {
  ...
  const result = await pool.query(
    ...
    [
      ...
      Array.isArray(praesenz_wochen) ? praesenz_wochen : null,
...
router.get('/gage-kategorien', async (_req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT id, label, kat_nr, beschreibung, abrechnungstyp, betrag, waehrung, lst_rg, sortierung
       FROM autorenplan_gage_kategorien ORDER BY kat_nr NULLS LAST, sortierung, erstellt_am`
    )
...
router.post('/gage-kategorien', async (req, res) => {
  const { label, kat_nr, beschreibung, abrechnungstyp, betrag, waehrung, lst_rg } = req.body
  if (!label?.trim()) return res.status(400).json({ error: 'label erforderlich' })
  try {
    const { rows } = await pool.query(
      `INSERT INTO autorenplan_gage_kategorien (label, kat_nr, beschreibung, abrechnungstyp, betrag, waehrung, lst_rg)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING id, label, kat_nr, beschreibung, abrechnungstyp, betrag, waehrung, lst_rg, sortierung`,
      [label.trim(), kat_nr ?? null, beschreibung || null, abrechnungstyp || 'pauschal', betrag ?? null, waehrung || 'EUR', lst_rg || 'rg']
    )
  ...
  try {
    const { rows } = await pool.query(
      `UPDATE autorenplan_gage_kategorien SET
         ...
         kat_nr          = $2,
         beschreibung    = $3,
         abrechnungstyp  = COALESCE($4, abrechnungstyp),
         betrag          = $5,
         waehrung        = COALESCE($6, waehrung),
         lst_rg          = COALESCE($7, lst_rg),
         aktualisiert_am = NOW()
       WHERE id = $8
       RETURNING id, label, kat_nr, beschreibung, abrechnungstyp, betrag, waehrung, lst_rg, sortierung`,
      [label ?? null, kat_nr ?? null, beschreibung ?? null, abrechnungstyp ?? null, betrag ?? null, waehrung ?? null, lst_rg ?? null, req.params.id]
    )
